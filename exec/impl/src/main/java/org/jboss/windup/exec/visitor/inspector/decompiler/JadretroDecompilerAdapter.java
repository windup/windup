 ...
 * 
 * JadRetro is a command-line utility that could helps to successfully decompile Java classes created by the Java
 * compilers of Java 1.4, Java 1.5 or later. JadRetro operates by transforming the specified Java class files (if
 * needed) into ones which could be processed correctly by an old Java decompiler (designed to work with classes of Java
 * 1.3 or earlier). JadRetro is not a decompiler itself, it is a class transformer helping some old (but good) Java
 * decompilers to convert more class files and/or generate more correct source code.
 * 
 * http://jadretro.sourceforge.net/
 * 
 * To use this adapter successfully the Jad decompiler must be setup on the environment. It can be downloaded from
...
public class JadretroDecompilerAdapter implements DecompilerAdapter
{
    ...
    private void executeJad(File classLocation, File sourceOutputLocation)
    {
        ...
        {
            throw new RuntimeException("Error running " + APP_NAME + " decompiler.  Validate that " + APP_NAME
                        + " is on your PATH.", e);
        }
    }
}
