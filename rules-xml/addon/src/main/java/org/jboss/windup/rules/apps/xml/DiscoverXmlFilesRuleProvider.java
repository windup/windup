...
public class DiscoverXmlFilesRuleProvider extends AbstractRuleProvider
{
    ...
    private void addXmlMetaInformation(GraphContext context, XmlFileModel file)
    {
        ...
        catch (Exception e)
        {
            ...
            {
                LOG.log(Level.WARNING, "Failed to parse xml entity: " + file.getFullPath() + ", as the file is empty.");
            }
            else
            {
                LOG.log(Level.WARNING, "Failed to parse xml entity: " + file.getFullPath() + ", due to: " + e.getMessage(),
                        e);
            }
