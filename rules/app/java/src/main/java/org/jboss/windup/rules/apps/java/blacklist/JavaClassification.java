...
/**
 * A class collects information about java classifications. If registered, it respond to all of the
 * {@link JavaScannerASTEvent} and create a vertex of the java classification, if the {@link ClassCandidate} contained
 * in the event contain the information stored in this instance.
 * 
 * @author mbriskar
 * 
 */
...
{
    @Inject
    private JavaClassService classService;
     ...
     * @return
     */
    JavaClassificationModel createClassificationModel(JavaScannerASTEvent event)
    {
        ClassCandidate classCandidate = event.getClassCandidate();
        JavaClassificationModel classificationModel = new GraphService<JavaClassificationModel>(event.getContext(),
                    JavaClassificationModel.class).create();
        for (ClassCandidateType type : classCandidateTypes)
        {
            GraphService<ClassCandidateTypeModel> graphService = new GraphService<ClassCandidateTypeModel>(
                        event.getContext(), ClassCandidateTypeModel.class);
            ClassCandidateTypeModel alreadyExistingTypeModel = graphService.getUniqueByProperty(
                        ClassCandidateTypeModel.PROPERTY_CLASS_CANDIDATE_TYPE, type);
            ...
            {
                classCandidateModel = new GraphService<ClassCandidateTypeModel>(event.getContext(),
                            ClassCandidateTypeModel.class).create();
            }
        ...
        classificationModel
                    .setReferencedJavaClassModel(classService.getUniqueByName(classCandidate.getQualifiedName()));
        classificationModel.setFileModel(event.getFileModel());
        if (links != null)
        {
            for (Link link : links)
            {
    ...
    public void evaluateASTEvent(JavaScannerASTEvent event)
    {
        ...
        if (regexPattern.matcher(qualName).matches())
        {
            ...
            {
                JavaClassificationModel classificationModel = createClassificationModel(event);
