...
public class RunWindupCommand implements Command, FurnaceDependent {
    ...
    private void runWindup(List<String> arguments) {
        ...
        {
            System.err.println("ERROR: " + "Version must be specified for target 'eap' (for example, 'eap8 or 'eap7')");
            return;
        }

        if ((targets == null || targets.isEmpty()) && !batchMode.get())
        {
            String target = Bootstrap.promptForListItem("Please select a target:", ruleProviderRegistryCache.getAvailableTargetTechnologies(), "eap8");
