 ...
 * here the running VM Bootclasspath is collected as the last one and not as the first one just like in the
 * original implementation in order to have "custom" JDK libraries used to resolve classes before the running
 * JVM ones.
 * For example: when running on JDK 11 and parsing a JDK 8 application, the classes and methods removed in JDK 11
 * won't be parsed and adding the 'rt.jar' from JDK 8 won't solve the issue because the JDK 11 running VM will "win"
 * when parsing.
 * With the changes in this parser, the above situation works fine and the JDK 8 application is successfully parsed.
 */
@SuppressWarnings({"rawtypes"})
public class WindupASTParser {
