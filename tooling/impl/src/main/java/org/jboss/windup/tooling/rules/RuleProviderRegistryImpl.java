...
public class RuleProviderRegistryImpl implements RuleProviderRegistry  
{
	...
	public void buildRuleProviders(RuleProviderRegistryCache ruleProviderCache) 
	{
        ...
        org.jboss.windup.config.metadata.RuleProviderRegistry cachedRuleProviderRegistry = ruleProviderCache.getRuleProviderRegistry(ruleLoaderContext);
        List<org.jboss.windup.config.RuleProvider> cachedRuleProviders = cachedRuleProviderRegistry.getProviders();
        
		for (org.jboss.windup.config.RuleProvider provider : cachedRuleProviders)
        {
            ...
            setFileMetaData(ruleProvider.getRulesPath(), ruleProvider);
            ...
            for (org.ocpsoft.rewrite.config.Rule rule : provider.getConfiguration(ruleLoaderContext).getRules())
            {
	...
	private void setFileMetaData(RulesPath rulesPath, RuleProvider ruleProvider) 
	{
		...
		try 
		{
			...
			ruleProvider.setRulesPath(rulesPath);

			if (ruleProvider.getRulesPath() != null) 
			{
