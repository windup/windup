...
public class RuleSubset extends DefaultOperationBuilder implements CompositeOperation, Parameterized, CompositeRule
{
    ...
    {
        AbstractRuleProvider ruleProvider = (AbstractRuleProvider) context.get(RuleMetadataTypes.RULE_PROVIDER);
    ...
    public void perform(Rewrite rewrite, EvaluationContext context)
    {
        ...
        for (int i = 0; i < rules.size(); i++)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (rule.evaluate(event, subContext))
                    {
                        ...
                        final Object ruleProviderDesc = ((RuleBuilder) rule).get(RuleMetadataTypes.RULE_PROVIDER);
                ...
                finally
                {
                    ...
                    if (ruleContext != null && ruleContext.containsKey(RuleMetadataTypes.AUTO_COMMIT))
                    {
                        autocommit = (Boolean) ruleContext.get(RuleMetadataTypes.AUTO_COMMIT);
                    }
                    if (autocommit)
                    {
            ...
            catch (RuntimeException ex)
            {
                ...
                if (ruleContext != null)
                {
                    Object origin = ruleContext.get(RuleMetadataTypes.ORIGIN);
                    if (origin != null)
                        exMsg += "\n  From: " + origin;
