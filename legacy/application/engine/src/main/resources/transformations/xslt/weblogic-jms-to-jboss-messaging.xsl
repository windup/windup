...
<xsl:stylesheet version="1.0" xmlns:wl="http://www.bea.com/ns/weblogic/90"
	...
	<xsl:template match="wl:queue">
		...
		<xsl:element name="mbean">
			...
			<xsl:attribute name="name"><xsl:text>jboss.messaging.destination:service=Queue,name=</xsl:text><xsl:value-of select="$queueName"/></xsl:attribute>
			...
			<xsl:element name="depends"><xsl:text>jboss.messaging:service=PostOffice</xsl:text></xsl:element>
		</xsl:element>
	</xsl:template>
	...
	<xsl:template match="wl:topic">
		...
		<xsl:element name="mbean">
			...
			<xsl:attribute name="name"><xsl:text>jboss.messaging.destination:service=Topic,name=</xsl:text><xsl:value-of select="$queueName"/></xsl:attribute>
