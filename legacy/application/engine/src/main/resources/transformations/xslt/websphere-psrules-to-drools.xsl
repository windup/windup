...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:rl="http://www.ibm.com/xmlns/prod/websphere/wbi/br/6.0.0">
...
template "</xsl:text><xsl:value-of select="@name"/><xsl:text>"
</xsl:text>
...
<xsl:text>rule "</xsl:text><xsl:value-of select="../@name"/><xsl:text> @{row.rowNumber}"</xsl:text>
<xsl:if test="../../rl:rulegroup"><xsl:text>
	ruleflow-group "</xsl:text><xsl:value-of select="../../rl:rulegroup"/><xsl:text>"</xsl:text></xsl:if>
...
<xsl:text>	</xsl:text><xsl:value-of select="@value"/><xsl:text>;
	...
	<xsl:template match="rl:parameterValue">
		<xsl:text>
		</xsl:text><xsl:value-of select="rl:name"/><xsl:text> = "</xsl:text><xsl:value-of select="rl:value/@value"/><xsl:text>"</xsl:text>
	</xsl:template>
