...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="weblogic-enterprise-bean[.//stateless-session-descriptor]">
		<session>
			...
			<local-jndi-name><xsl:text>java:</xsl:text><xsl:value-of select="jndi-name" /></local-jndi-name>
			<xsl:if test="stateless-session-descriptor/stateless-clustering/stateless-bean-is-clusterable = 'True' or stateless-session-descriptor/stateless-clustering/stateless-bean-is-clusterable = 'true'">
				<clustered>
	...
	<xsl:template match="weblogic-enterprise-bean[.//entity-descriptor]">
		<entity>
			...
			<xsl:if test="entity-descriptor/entity-clustering/home-is-clusterable = 'True' or entity-descriptor/entity-clustering/home-is-clusterable = 'true'">
				<clustered>
					<xsl:text>true</xsl:text>
				</clustered>
			</xsl:if>
	...
	<xsl:template match="weblogic-enterprise-bean[.//message-driven-descriptor]">
		<message-driven>
			<ejb-name><xsl:value-of select="ejb-name" /></ejb-name>
			<destination-jndi-name><xsl:text>/topic/</xsl:text><xsl:value-of select="message-driven-descriptor/destination-jndi-name" /></destination-jndi-name>
			<xsl:apply-templates select="reference-descriptor"/>
		</message-driven>
	</xsl:template>
