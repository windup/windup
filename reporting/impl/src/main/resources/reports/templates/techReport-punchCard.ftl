            ...
            <h1>
                <div class="main">${reportModel.reportName}
                    <i class="glyphicon glyphicon-info-sign" data-toggle="tooltip" data-placement=right
                       title="${reportModel.description}"></i></div>
                <#if reportModel.projectModel??>
            ...
            <table class="technologiesPunchCard">
                ...
                <tr class="headersSector">
                    ...
                    <#list sectorTags as sector>
                        <td colspan="${ (iterableToList(sector.designatedTags)?size-1)?c}"
                            class="sectorHeader sector${sector.title}">
                    ...
                    <#list sectorTags as sectorTag >
                        ...
                        <#list sortedBoxTags as boxTag >
                            <#if !isTagUnderTag(boxTag, placeTagsParent) >
                                ...
                                <td class="sector sector${sectorTag.title}">
                                    <div>${boxTag.title!}</div>
                                </td>
                            </#if>
                        </#list>
                    </#list>
                    <td class="sector sectorStats sizeMB">
                        <div>Size (MB)</div>
                    </td>
                    <td class="sector sectorStats libsCount">
                        <div>Libraries</div>
                    </td>
                    <td class="sector sectorStats storyPoints">
                        <div>Mandatory (SP)</div>
                    </td>
                    <td class="sector sectorStats storyPoints">
                        <div>Cloud Mandatory (SP)</div>
                    </td>
                    <td class="sector sectorStats storyPoints">
                        <div>Potential (Count)</div>
                    </td>
                    <#-- this td is needed for scrollbar positioning -->
                    <td class="scrollbar-padding"></td>
                </tr>
                            ...
                            <#list sectorTags as sectorTag>
                                ...
                                <#list sortedBoxTags as boxTag>
                                    ...
                                    <#-- count: ${count?c}   max: ${maxForThisBox?c}   getLogaritmicDistribution(): ${ log?c } x 5 = ${ log * 5.0 } -->
                                        <td class="circle size${ log?is_number?then((log * 5.0)?ceiling, "X")} sector sector${sectorTag.title} table-tooltip"
                                            data-count="${countInteger?c}">
                                                            ...
                                                            <#if (stat.occurrenceCount > 0) >
                                                                <div class="row">
                                                                    <#if (countTooltipRows == 0)>
                                                                        <div class="tooltiptext-tech-name-header col-md-9">
                                                                            Total
                                                                        </div>
                                                                        <div class="tooltiptext-tech-count-header col-md-3">${stat.occurrenceCount}</div>
                                                                    <#else>
                                                                        <div class="tooltiptext-tech-name col-md-9">${stat.name}</div>
                                                                        <div class="tooltiptext-tech-count col-md-3">${stat.occurrenceCount}</div>
                                                                    </#if>
                                                                </div>
                                ...
                                <td class="sector${sectorTag.title}Summary sectorSummary" data-count="${countSector?c}"
                                    hidden></td>
                            </#list>
                            <#if isFileADirectory(appProject.rootFileModel)>
                                <td class="sectorStats sector sizeMB"
                                    data-count="${(appProject.rootFileModel.getDirectorySize()?c)!0}">
                                    ${ ( (appProject.rootFileModel.getDirectorySize() / 1024 / 1024)?string["0.##"] )! }
                                </td>
                            <#else>
                                <td class="sectorStats sector sizeMB"
                                    data-count="${(appProject.rootFileModel.retrieveSize()?c)!0}">
