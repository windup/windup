...
<div class="navbar navbar-inverse navbar-fixed-top" id="main-navbar" style="display: none">
    ...
    <#list reportModel.projectEdges as toProjectEdge>
        ...
        <#if applicationIndex??>
            <div class="navbar-collapse collapse navbar-responsive-collapse project-specific"
                 data-project-id="${toProjectEdge.projectModel.getElement().id()?c}">
            ...
            <h1>
                <div class="main">Source Report
                    <i class="glyphicon glyphicon-info-sign" data-toggle="tooltip" data-placement=right
                       title="This report displays what Tackle Analysis found in individual files. Each item is shown below the line it was found on, and next to it, you may find a link to the rule which it was found by."></i>
                </div>

                <#list reportModel.projectEdges as toProjectEdge>
                    <div class="path project-specific"
                         data-project-id="${toProjectEdge.projectModel.getElement().id()?c}">
                    ...
                    <div class="panel-body" style="overflow: auto;">
                        ...
                        <div class="info" style="margin-left: 95pt;">
                            ...
                            <#if (techTagsForFile?size + tagsForClassificationsAndHints?size) &gt; 0>
                                <h4>Technologies</h4>
                                <div class="technologies" style="overflow: auto">
                                    <!-- "auto" to contain all the tags. -->
                                    <#list techTagsForFile as techTag>
                                        <span class="label label-${(techTag.level! == 'IMPORTANT')?then('danger','info')}"
                                              title="${techTag.level}">${techTag.name}</span>
                                    </#list>
                                    ...
                                    <#items as item>
                                        <#if item.classification??>
                                            <li>
                                                ...
                                                <#if item.description??>
                                                    <div class="desc">${item.description}</div></#if>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $("pre").snippet("${reportModel.sourceType}", {
            style: "ide-eclipse",
            showNum: true,
            boxFill: "#ffeeb9",
            box: "${reportModel.sourceBlock}"
        });
        ...
        = 'inline-source-comment green tag-${hintClasses}' > <#t>
        ...
        = 'inline-comment-heading' > <#t>
            < strong

        class

        = 'notification ${effortPointsToCssClass(hintLine.effort)}' > <#t>
