...
public class ZipEntryMeta extends FileMeta {
	private static final int BUFFER = 2048;
	private static final Log LOG = LogFactory.getLog(ZipEntryMeta.class);
	
	private ZipEntry zipEntry;

	@Override
	public File getFilePointer() {
		if(!(this.getArchiveMeta() instanceof ZipMeta)) {
			throw new IllegalStateException("ZipEntryMeta must be child of Zip.");
		}
		
		ZipMeta parent = (ZipMeta)this.getArchiveMeta();
