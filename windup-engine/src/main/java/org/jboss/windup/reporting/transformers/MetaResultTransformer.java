...
public abstract class MetaResultTransformer<T extends ResourceMeta> {
	...
	protected boolean buildVendorResult(ArchiveMeta meta) {
		for (AbstractDecoration result : meta.getDecorations()) {
	...
	protected String buildTitle(T meta, File rootDirectory) {
		...
		if(meta instanceof FileMeta) {
			String starter = ((FileMeta)meta).getArchiveMeta().getRelativePath();
