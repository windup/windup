...
public class JavaMetaTransformer extends GenericMetaTransformer<JavaMeta> {
	private static final Log LOG = LogFactory.getLog(JavaMetaTransformer.class);

	protected static int calcTotalChanges(JavaMeta result) {
		int totalChanges = 0;
	...
	@Override
	protected String buildTitle(JavaMeta meta, File rootDirectory) {
		return meta.getQualifiedClassName();
	}
	
	@Override
	protected String buildSummary(JavaMeta meta) {
