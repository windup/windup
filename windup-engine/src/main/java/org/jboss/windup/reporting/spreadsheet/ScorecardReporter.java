...
public class ScorecardReporter implements Reporter {
	...
	protected File generateScorecardName(ArchiveMeta archive, File reportDirectory) {
	...
	public void process(ArchiveMeta archive, File reportDirectory) {
		...
		List<ArchiveMeta> results = unwind(archive);
		...
		try {
			...
			int rownum = 1;
			for (ArchiveMeta result : results)
			{
				...
				double estimate = 0;
				// calculate the nodes..
				for (FileMeta ir : result.getEntries()) {
	...
	private List<ArchiveMeta> unwind(ArchiveMeta result) {
		List<ArchiveMeta> unwound = new LinkedList<ArchiveMeta>();
		unwind(result, unwound);
		
		return unwound;
	}
	
	private void unwind(ArchiveMeta result, List<ArchiveMeta> unwound) {
		for(ArchiveMeta meta : result.getNestedArchives()) {
			unwind(meta, unwound);
		}
		unwound.add(result);
	}
