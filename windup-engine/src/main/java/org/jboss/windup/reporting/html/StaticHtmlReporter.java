...
import org.jboss.windup.decorator.archive.ClassesProvidedDecorator;
...
public class StaticHtmlReporter implements Reporter {
	...
	protected void writeStaticResourceReports(ArchiveReport archive, File reportDirectory) {
		if(LOG.isDebugEnabled()) {
			LOG.debug("Recursing ArchiveMeta: "+archive.getRelativePathFromRoot());
		}
		...
		if(LOG.isDebugEnabled()) {
			LOG.debug("Processing ArchiveMeta: "+archive.getRelativePathFromRoot());
		}
	...
	protected ArchiveReport toArchiveReport(ArchiveMeta archive, File reportDirectory) {
		...
		for(ArchiveMeta meta : archive.getNestedArchives()) {
			ArchiveReport temp = toArchiveReport(meta, reportDirectory);
			report.getNestedArchiveReports().add(temp);
		}
		
		for(String key : archive.getContext().keySet()) {
			if(StringUtils.equals(key, ClassesProvidedDecorator.MISSING_DEPENDENCIES_TO_APPLICATION_CLASSES))
			{
				Map<String, List<String>> classloaderIssues = (Map<String, List<String>>)archive.getContext().get(key);
				ClasspathReport cpr = new ClasspathReport(classloaderIssues);
				cpr.setTitle("Classloader Report");
				cpr.setSummary("The following issues have been discovered as potential problems with the classpath.");
				cpr.setRelativePathFromRoot(report.getRelativePathFromRoot());
				cpr.setRelativePathToRoot(report.getRelativePathToRoot());
				
				report.getAdditionalInformationReports().add(cpr);
			}
			else {
				LOG.warn("Unknown plugable report with context: "+key);
			}
		}
		
		
		for(FileMeta fileMeta : archive.getEntries()) {
