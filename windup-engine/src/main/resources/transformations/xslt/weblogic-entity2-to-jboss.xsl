...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:output method="xml" indent="yes" 
		doctype-system="http://www.jboss.org/j2ee/dtd/jbosscmp-jdbc_4_0.dtd" 
	...
	<!-- 
		<xsl:element name="defaults">
		 ...
		 <xsl:if test="./create-default-tables/text() != 'Disabled'">
		 	<xsl:element name="create-table">true</xsl:element>
		 </xsl:if>
		...
		<xsl:element name="jbosscmp-jdbc">
			<xsl:element name="defaults">	
				<xsl:choose>
					...
					<!--
					CreateOnly:
					The EJB container automatically generates the table upon 
					detecting changed schema.  The container attempts to create 
					the table based on information found in the deployment files 
					and in the bean class. If table creation fails, a 
					'Table Not Found' error is thrown, and the user must create 
					the table manually.
					-->
					<xsl:when test="create-default-dbms-tables = 'CreateOnly'">
						<xsl:element name="create-table">true</xsl:element>
						<xsl:element name="alter-table">false</xsl:element>
						<xsl:element name="remove-table">false</xsl:element>
					</xsl:when>

					<!--
					DropAndCreate:
					The EJB container automatically generates the table upon 
					detecting changed schema.  The container drops and creates 
					the table during deployment if columns have changed. 
					The container does not save data.
					-->
					<xsl:when test="create-default-dbms-tables = 'DropAndCreate'">
						<xsl:element name="create-table">true</xsl:element>
						<xsl:element name="alter-table">true</xsl:element>
						<xsl:element name="remove-table">false</xsl:element>
					</xsl:when>
					...
					The EJB container automatically generates the table upon 
					detecting changed schema.  The container drops and creates 
					the table during deployment whether or not columns have 
					changed. The container does not save the data.
					-->
					<xsl:when test="create-default-dbms-tables = 'DropAndCreateAlways'">
						<xsl:element name="create-table">true</xsl:element>
						<xsl:element name="alter-table">true</xsl:element>
						<xsl:element name="remove-table">true</xsl:element>
					</xsl:when>

					<!--
					AlterOrCreate:
					The EJB container automatically generates the table upon 
					detecting changed schema.  The container creates the table 
					if it does not yet exist. If the table does exist, the 
					container alters the table schema. Table data is saved.
					-->
					...
					<!-- JBoss Defaults
					<xsl:otherwise>
						<xsl:element name="create-table">false</xsl:element>
						<xsl:element name="alter-table">false</xsl:element>
						<xsl:element name="remove-table">false</xsl:element>
					</xsl:otherwise>
					-->
	...
	<xsl:template match="weblogic-rdbms-bean">
		<xsl:element name="entity"> 
			<xsl:element name="ejb-name"><xsl:value-of select="./ejb-name"/></xsl:element>
			<xsl:element name="datasource">java:/<xsl:value-of select="data-source-name"/></xsl:element>
			<xsl:element name="datasource-mapping">TODO: Replace with Mapping</xsl:element>
	...
	<xsl:template match="table-name">
			<xsl:element name="table-name"><xsl:value-of select="."/></xsl:element>
	</xsl:template>
	...
	<xsl:template match="field-map">
		<xsl:element name="cmp-field">
        		<xsl:element name="field-name"><xsl:value-of select="cmp-field"/></xsl:element>
        		<xsl:element name="column-name"><xsl:value-of select="dbms-column"/></xsl:element>
      		</xsl:element>
	</xsl:template>
	...
	<xsl:template match="weblogic-rdbms-relation">
		<xsl:element name="ejb-relation">
			<xsl:comment>TODO: Complete ejb relationship</xsl:comment>
