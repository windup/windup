...
public class WindupTestCase {
	...
	protected void runArchiveMetaTest(String archivePath, String[] expectedArchiveDecorations, Map<String, String[]> expectedArchiveEntryDecorations, String outputPath) throws IOException {
		...
		ArchiveMeta archiveMeta = this.getArchiveMeta(input, output);
	...
	/**
	 * <p>
	 * Run a test on a single {@link ArchiveMeta}
	 * </p>
	 * 
	 * @param archiveMeta
	 *            {@link ArchiveMeta} to test
	 * @param expectedArchiveDecorations
	 *            expected decorations on the archive
	 * @param expectedArchiveEntryDecorations
	 *            map of file paths to their expected decorations
	 */
	private void runArchiveMetaTest(ArchiveMeta archiveMeta, String[] expectedArchiveDecorations, Map<String, String[]> expectedArchiveEntryDecorations) {
		Assert.assertNotNull("ArchiveMeta should not be null", archiveMeta);
		...
		Set<FileMeta> entries = new HashSet<FileMeta>(archiveMeta.getEntries());
		...
		for (String expectedEntryFilePath : expectedEntryPaths) {
			...
			Iterator<FileMeta> entriesIter = entries.iterator();
			while (entriesIter.hasNext()) {
				FileMeta entry = entriesIter.next();
	 ...
	 * 
	 * @param input
	 *            {@link File} to generate {@link ArchiveMeta} for
	 * @param outputDir
	 *            Optional. The output directory to be used when generating the
	 *            meta
	 * 
	 * @return {@link ArchiveMeta} generated for the given {@link File}
	 * 
	 * @throws IOException
	 *             can happen when trying to read files
	 */
