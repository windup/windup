	...
	public void visitXmlResource(XmlResource entry) {
		...
		try {
			...
			for(int i=0, j=nodes.getLength(); i<j; i++) 
			{
				...
				final String dependencyGroupId = XmlUtil.xpathExtract(document, "//pom:groupId", namespaces);
				final String dependencyArtifactId = XmlUtil.xpathExtract(document, "//pom:artifactId", namespaces);
				final String dependencyVersionId = XmlUtil.xpathExtract(document, "//pom:version", namespaces);
