...
import { EjbRemoteServiceDto } from "@app/api/application-remote-services";
...
export interface IEJBRemoteTableProps {
  applicationId: string;
}

export const EJBRemoteTable: React.FC<IEJBRemoteTableProps> = ({
  applicationId,
}) => {
  // Filters
  ...
  const { pageItems, filteredItems } = useTable<EjbRemoteServiceDto>({
    items: beans,
  ...
  return (
    <>
      <ConditionalRender
        when={allRemoteServicesQuery.isLoading}
        then={
          <Bullseye>
            <Spinner />
          </Bullseye>
