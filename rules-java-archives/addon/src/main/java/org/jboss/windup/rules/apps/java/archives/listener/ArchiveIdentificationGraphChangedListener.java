...
public final class ArchiveIdentificationGraphChangedListener implements GraphListener
{
    ...
    private boolean shouldBeIgnored(ArchiveModel archive) {
        ...
        } else if ((Boolean) Optional.ofNullable(context.getOptionMap().get(AnalyzeKnownLibrariesOption.NAME)).orElse(false)) {
            // If the forceAnalysisOfKnownLibraries option is enabled, do not ignore it
            shouldBeIgnored = false;
        } else {
